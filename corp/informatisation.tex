...
La réalisation de projet en lien avec la ville connectée nécessite l'utilisation d'un grand nombre de données.
...
Ces données doivent être collecté et envoyer sur des serveurs qui les distribuerons à des entreprises de service,
malheureusement, il n'existe pas une norme unique pour les objets connecté.

\subsection{Les protocoles objet}

Il existe quatres grandes normes de communication aujourd'hui lorsque l'on parle d'objets connecté.

Zigbee est basé sur le standard IEEE 802.15.4.
C'est un protocole efficace qui consomme peu d'énergies, mais qui possède une courte porté.
Ses longueurs d'ondes sont des les fréquences utilisé par l'ISM pour Industrie, Scientifique et Médicale.
Elles ont une fréquence de 2.4 GHz, 868 MHz et 928 MHz ont une possibilité de communication en duplex.
...
Le Wi-Fi est aujourd'hui la norme la plus largement utilisé et implanté dans le monde.
Basé sur le standard IEEE 802.11, il couvre une large part des bandes réseaux pour des
besoins différents.
Le Wi-Fi IEEE 802.11b couvre les bandes 2.4 GHz avec un débit de donnée allant jusqu'a 11 Mbps.
...
Il peux couvrir une distance de 30 mètres grace a sa fréquence de 908 MHz.
...
Le débit vas de 9.6 Kbps à 40 Kbps.

Ces protocoles répondent tous au même besoin, communiquer entre un objet que l'on appellera capteur, et un
serveur.

Si l'on prend l'exemple d'un parking, celui-ci peux avoir un capteur par place disponible pour indiquer
les places libres ou non.
Ces capteurs devrons communiquer les informations à un serveur qui fournira l'information
aux usager.
...
Lorsqu'une donnée, qui correspond aux contraintes d'un consommateur, est ajouté, modifié ou supprimé,
